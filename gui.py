...
class GameScreen(urwid.Frame):
    def __init__(self, master):
        ...
        self.status_widget = SelectableListBox(urwid.SimpleListWalker(widgets))
        self.location_widget = SelectableListBox(urwid.SimpleListWalker(widgets))
        self.inventory_widget = SelectableListBox(urwid.SimpleListWalker(widgets))
        self.equipment_widget = SelectableListBox(urwid.SimpleListWalker(widgets))
        self.mission_widget = SelectableListBox(urwid.SimpleListWalker(widgets))
        self.screens = [self.status_widget, self.inventory_widget, self.equipment_widget, self.location_widget, self.mission_widget]
        ...
        labels = []
        for b in ("Status", "Inventory", "Equipment", "Location", "Mission"):
        ...
        self.active_widget = self.status_widget
        super(GameScreen, self).__init__(urwid.LineBox(self.active_widget, title=""), urwid.LineBox(urwid.BoxAdapter(self.players_widget, 12), title=""),  self.buttons_column , focus_part="header")
    ...
    def update_screen(self, protagonist):
        ...
        self.active_widget = self.screens[self.buttons_column.focus_position]
        self.contents["body"] = (urwid.LineBox(self.active_widget, title=""), None)
        self.contents["header"][0].set_title(protagonist.name)
        self.refresh_active_widget(players[index])
        
        
    def refresh_active_widget(self, player):
        if self.active_widget == self.status_widget:
            ...
            self.inventory_widget_state = "normal"
            self.inventory_widget_index = 1
        elif self.active_widget == self.location_widget:
            ...
            self.inventory_widget_state = "normal"
            self.inventory_widget_index = 1
        elif self.active_widget == self.inventory_widget:
            self.refresh_inventory_widget(player)
            self.contents["body"][0].set_title("Inventory")
            self.status_widget_state = "normal"
            self.equipment_widget_state = "normal"
            self.mission_widget_state = "normal"
        elif self.active_widget == self.mission_widget:
            ...
            self.inventory_widget_state = "normal"
            self.inventory_widget_index = 1
        elif self.active_widget == self.equipment_widget:
...
Location: {} --> {}""".format(player.generation, player.world.generation, player.race.name, player.job.name, player.level, player.exp, state,
        ...
        try:
            index = self.status_widget.focus_position
            columns.focus_position = self.status_widget.body[:][index].focus_position
        except:
            pass        
        widgets = [urwid.Text(data), columns]
        self.status_widget.body[:] = widgets
    ...
    def refresh_location_widget(self, player):
        ...
        try:
            index = self.location_widget.focus_position
            button_line.focus_position = self.location_widget.body[:][index].focus_position
        except:
            pass
        ...
        self.location_widget.body[:] = widgets
    
    def refresh_mission_widget(self, player):
        def show_missions(button):
            self.mission_widget_state = "show"
        ...
        widgets = []
        if self.mission_widget_state == "normal":
        ...
        self.mission_widget.body[:] = widgets
        
    def refresh_inventory_widget(self, player):
        def show_item(item, button):
            self.inventory_widget_state = item.id
        ...
        if self.inventory_widget_state == "normal":
            for i in items:
                ...
                columns = SelectableColumns(line, dividechars=2)
                try:
                    index = self.inventory_widget.focus_position
                    columns.focus_position = self.inventory_widget.body[:][index].focus_position
                except:
                    pass
                
                widgets.append(columns)
        else:
            i = next((it for it in items if it.id == self.inventory_widget_state), None)
            if i == None:
                self.inventory_widget_state = "normal"
            else:
    ...
    def refresh_equipment_widget(self, player):
        ...
        def send_unequip(button):
            if self.equipment_widget_state in player.equipment:
                player.unequip(player.equipment[self.equipment_widget_state])
        ...
        if self.equipment_widget_state == "normal":
            ...
            t = urwid.Text("{}".format(self.equipment_widget_state), align='center')
            widgets.append(t)
            b = urwid.Button("None")
            b._label.align = 'center'
            w = urwid.AttrMap(b,None,"line")
            ...
            columns = SelectableColumns(line, dividechars=2)
            widgets.append(columns)
            eqs = [i for i in player.inventory if i.type == self.equipment_widget_state and i.requisites(player)]
            for i in eqs:
                bonus = ",".join([" {}: {}".format(bns, i.bonus[bns]) for bns in i.bonus if i.bonus[bns]!=0]) + " " + ",".join([" {}: {} ".format(ab, i.abilities[ab].name) for ab in i.abilities] )
        ...
        self.equipment_widget.body[:] = widgets
